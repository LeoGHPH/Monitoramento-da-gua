class Usuario:
    def _init_(self, id, nome, email, senha, perfil):
        self.id = id
        self.nome = nome
        self.email = email
        self.senha = senha
...
class Medidor:
    def _init_(self, id, numero_serie, localizacao, tipo):
        self.id = id
...
class Leitura:
    def _init_(self, id, medidor_id, data_leitura, consumo, status):
        self.id = id
...
class Consumo:
    def _init_(self, id, leitura_id, valor, unidade):
        self.id = id
...
class Alerta:
    def _init_(self, id, medidor_id, tipo, limite, status):
        self.id = id
        self.medidor_id = medidor_id
        self.tipo = tipo
        self.limite = limite
...
class HistoricoConsumo:
    def _init_(self, id, medidor_id, data_inicio, data_fim, consumo_total):
        self.id = id
        self.medidor_id = medidor_id
...
class ConfiguracaoSistema:
    def _init_(self, id, limite_consumo_diario, limite_consumo_mensal, unidade_medida):
        self.id = id
...
class Notificacao:
    def _init_(self, id, alerta_id, usuario_id, mensagem, data_envio):
        self.id = id
...
class SistemaMonitoramento:
    ...
    def enviar_notificacao(self, alerta_id, usuario_id):
        alerta = next((a for a in self.alertas if a.id == alerta_id), None)
        if alerta:
            notificacao = Notificacao(len(self.notificacoes) + 1, alerta_id, usuario_id, "Consumo alto detectado!", datetime.now())
...
usuario1 = Usuario(1, "João", "joao@example.com", "senha123", "admin")
sistema.adicionar_usuario(usuario1)

medidor1 = Medidor(1, "123456", "Casa", "residencial")
sistema.adicionar_medidor(medidor1)

leitura1 = Leitura(1, 1, datetime.now(), 100, "valido")sistema.adicionar_leitura(leitura1)

sistema.gerar_consumo(1)
sistema.gerar_alerta(1)
sistema.enviar_notificacao(1, 1)
